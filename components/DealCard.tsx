...
function closeDateColor(deal: ParsedDeal): string {
  ...
  if (diff === 0) return "#F5A623";
  if (Math.abs(diff) <= 7) return "#F5A623";
  return "#6B7F96";
}
...
export default function DealCard({ deal, onClick, onAIClick, onResearchClick }: DealCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Header: stage pill + age badge */}
      <div style={{ padding: "8px 10px 0 12px" }}>
      ...
      {/* Body: value + close date + tags */}
      <div style={{ padding: "6px 10px 0 12px", flex: 1 }}>
        {/* Value */}
        <p
          style={{
            fontSize: 20,
